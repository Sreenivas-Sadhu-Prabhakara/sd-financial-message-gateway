...
| **Asset Type** | Financial Message |
| **Control Record** | Financial Message Operating Session |
...
| POST | `/v1/financial-message-operating-session/initiate` | Initiate |
| GET | `/v1/financial-message-operating-session` | Retrieve (list) |
| GET | `/v1/financial-message-operating-session/{crId}/retrieve` | Retrieve |
| PUT | `/v1/financial-message-operating-session/{crId}/update` | Update |
| PUT | `/v1/financial-message-operating-session/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/financial-message-operating-session/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-financial-message-gateway:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-financial-message-gateway ./helm -n bian-operations
```

Exposed through the platform Gateway at path prefix `/sd-financial-message-gateway` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
